...
- **Bilibili 登录弹幕队列兼容修复（源码未发布）**：Issue #872 对应的 3.1.4 认证包缺少当前队列声明，也未回传要求确认的消息。当前源码已增加 `support_ack`、每连接 queue UUID、`scene=room`、operation 24 ACK，并让 uid 优先绑定同一 Cookie 的 `DedeUserID`。有效红灯 7 PASS / 3 FAIL；协议 11/11、七文件相邻回归 57/57、最终 analyze 及严格公开实连通过。报告者登录态 Windows 复验继续，详见[Issue #872 专项审计](docs/ISSUE_872_BILIBILI_LOGGED_IN_DANMAKU_AUDIT_2026_09_19.md)。
- **斗鱼弹幕完整性修复（源码未发布）**：新安装、3.1.4 升级和缺字段备份默认显示平台收到的全部非空房间聊天，不再把缺少 `dms` / `if` 装饰标记当作不可见；疑似自动消息过滤保留为明确选择，设置即时生效并提示也可能隐藏普通聊天。有效红灯 16 PASS / 2 FAIL，最终相邻回归 53/53 与全库 analyze 通过；71415 审查时离线，实时网页数量对照继续，详见[Issue #871 / #845 专项审计](docs/ISSUE_871_DOUYU_CHAT_COMPLETENESS_AUDIT_2026_09_19.md)。
