...
- **最新稳定版**：[v2.0.36](https://github.com/wzgrx/pure_live/releases/tag/v2.0.36)
- **当前版本**：`2.1.0+48`（阶段性大更新）
- **上游同步基线**：`liuchuancong/pure_live@77c2c9e`（2026-08-17）
...
- **直播平台**：Bilibili、虎牙、斗鱼、快手、抖音、网易 CC、Twitch。
...
- **搜索与互动**：跨平台原生直播搜索、滚动分页加载，以及保存在本机的昵称、头衔、弹幕输入、体验币、七平台身份徽章、礼物目录、等级风格和画面礼物效果；“设置 → 本地用户与互动”提供独立总开关。
- **观看口径**：热度、真实在线人数、累计观看分别存储和标注；抖音、快手、网易 CC、Twitch 可显示明确并发人数，虎牙/Bilibili/斗鱼按平台热度显示；“设置 → 通用 → 观看数据与排行口径”可切换排行方式并管理支持平台。
...
独立设置页提供实时样式预览和恢复默认值；手机端把预览固定在上方、只滚动下方参数，调整长设置时画面变化始终可见。即使主播放器的全局弹幕已关闭，小窗弹幕设置入口仍保持可见。
...
Linux、macOS 与 iOS 的阶段构建由“全部平台”手动工作流生成；iOS 附件是设备 `.app` 编译归档，签名与 IPA 封装在持有 Apple 证书的环境中完成。阶段更新范围见 [v2.1.0 阶段更新说明](docs/STAGE_UPDATE_2_1_0.md)。
...
| Bilibili 弹幕昵称显示为 `某***` | 平台会对访客连接返回脱敏昵称；应用会显示来源提示，登录对应平台后优先读取其返回的完整昵称 |
