...
void main() {
  test('room command-line payload round-trips without platform response blobs', () {
    ...
    final args = WindowsMultiInstanceLauncher.buildArguments(
      room: room,
      processId: 42,
  ...
  test('instance ids are sanitized and malformed room payloads are ignored', () {
    expect(
      WindowsMultiInstanceLauncher.instanceIdFromArgs(const ['--instance=room ../A-1']),
      'room.A-1',
    );
    expect(WindowsMultiInstanceLauncher.instanceIdFromArgs(const ['--instance=../../']), isEmpty);
    expect(
      WindowsMultiInstanceLauncher.instanceIdFromArgs(const ['--instance=NUL']),
      'instance_NUL',
    );
  ...
  test('room payload carries canonical offline state instead of a stale legacy boolean', () {
    ...
    final decoded = WindowsMultiInstanceLauncher.roomFromArgs([
      WindowsMultiInstanceLauncher.encodeRoomArgument(room),
    ]);
