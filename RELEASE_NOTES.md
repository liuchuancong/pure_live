...
v3.1.2 build 4115 是 Android / Windows 全屏一致性补丁。本轮没有同步上游代码；根因来自 Windows 隐藏标题栏初始化与 `window_manager 0.5.2` 原生 `frameless` 状态的交互，不是 v3.1.1 多画面业务代码。旧版会把 Dart 侧标记为全屏，但原生窗口没有改变样式或边界，因此仍保留标题栏、任务栏和普通页面布局。
