...
v3.0.10 build 4098 是 Android 横屏全屏竖屏展示修正版。本轮只构建 `arm64-v8a` Release APK；其他平台继续使用既有安装包。

## 根因

- 竖屏普通页与应用小窗的容器已经按 9:16 建立，所以即使播放器继续读取全局画面填充选项，问题会被容器比例掩盖；横屏全屏是 16:9 宽容器，`fill` 会直接拉伸，`cover` 会裁掉竖屏上下内容。
- 横屏全屏虽然已经创建模糊封面层，但 MediaKit `Video.fill`、Fijk `color` 及兼容内核的表面仍使用黑色。这个全屏原生画布位于背景之上，完全遮住了背景，因此代码存在却看不到。
- 既有测试只确认背景组件存在，没有检查 `BoxFit.contain` 和透明 fill 是否真正传到原生播放器，导致第一层 UI 修复未覆盖渲染末端。

## 修复

- 竖屏内容进入横屏全屏后强制使用 `BoxFit.contain`，不再继承普通直播间的 fill/cover；普通横屏继续遵循用户设置。
- `UnifiedPlayer` 新增显式 `fit + fill` 视口合同，并由 MediaKit、Fijk 和 BetterPlayer 三套内核实现。竖屏全屏传入透明表面，使居中视频之外的区域显示暗化模糊封面。
- 背景资源按详情封面、房间封面、详情头像、房间头像回退，资源缺失或下载失败时保留暗色渐变。
- 全屏画面适配控件在受保护的竖屏展示中显示“包含/默认”状态，避免 UI 显示 fill 而实际为 contain；退出全屏后恢复用户原设置。

## 跨平台边界与验证

- 方向识别在站点解析之后读取解码器显示尺寸，因此 Bilibili、斗鱼、虎牙、抖音、快手、网易 CC、Twitch、SOOP、YY 和自定义源的直接竖屏流走同一检测路径，不依赖平台名称。
- 16:9 传输画布内嵌竖屏节目需要两次一致的真实帧黑边证据；当前默认 MediaKit 路径具备这一探测，元数据缺失或混合画面仍可使用直播间手动方向覆盖。
- 回归覆盖全屏 fit/fill 透传、背景回退、跨平台 9:16 分类、普通横屏隔离、移动端单层缩放、音频模式与播放器生命周期。
