...
- Android 从系统画中画返回全屏或竖屏时，主动重建当前房间弹幕传输，同时保留正在显示的画面弹幕，避免同房间会话键仍存在但 WebSocket 已休眠。
- 弹幕列表在 PiP 交还页面时重新获取当前 500 条快照并恢复实时跟随，清理由视口拆装产生的误暂停状态。
- 弹幕列表仅在 80 ms UI 批次落地时复制可见快照；本地消息仍立即同步到列表与画面。
- 弹幕文本解析、段落布局、图片和表情 Token 缓存全部改为有界 LRU；主画面和小窗分别使用符合轨道规模的缓存/对象池上限。

## 启动刷新与横屏交互

- 收藏房间在控制器初始化阶段立即核验，不再等待首帧；每个有界批次完成后立即更新开播状态，已下播房间无需等全部平台请求结束。
