...
v2.3.0 聚焦系统画中画返回后的弹幕会话、启动数据新鲜度、横屏操作完整性，以及 Windows/Android 长时间播放的资源边界；维护分支已同步至上游 `67a24a24`，包含已合并的 PR #762、虎牙播放地址 PR #766、播放器引擎访问接口、issue #768 导航保底修复与 media_kit 路由跳转崩溃修复。

## 弹幕生命周期

- Android 从系统画中画返回全屏或竖屏时，由持续存活的直播控制器发布恢复 revision；系统转场未结束会自动重试，已有重连时保留并串行执行新请求。
- 弹幕列表重建后在首帧重新获取当前 500 条快照、提交待处理批次并恢复实时跟随；Android 的方向型视口通知不再被误判为用户上滑。
...
- media_kit/mpv 直播前向缓存限制为 64 MiB、回看缓存限制为 8 MiB，保留 3 秒预读，减少长时间直播的原生内存高水位。
- Windows 恢复鼠标拖动滚动，并保留滚轮脉冲合并动画；主画面弹幕限制高峰入场频率、可见数量和过期队列。
...
- 170 项单元/Widget 测试及 26/26 平台公开接口探测通过。
- Windows x64 release 已完成热门平台切换、封面回填、2K60 播放、实时弹幕与资源采样；Android arm64 使用持久发布证书正式签名并核对包名、ABI、版本和校验和。
- Android arm64 热修包从当前 `master` 的干净提交重新构建并使用持久发布证书签名；Windows、Linux、macOS 与 iOS 继续保留本阶段已发布归档，后续全平台周期再统一重建。
