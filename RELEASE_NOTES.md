...
本版本是 Windows 高刷新率、Bilibili 热门封面和桌面交互性能专项更新，并同步上游 `ca9e0d96`。

## Bilibili 热门与封面

- 热门列表由持续触发 `-352` 风控的旧分区接口切换到当前匿名推荐接口，并提供有界重试与旧匿名接口回退。
- 推荐响应解析兼容当前与回退结构，接口异常不再被空数据访问掩盖成含糊的“网络请求失败”。
- Bilibili 图片域名补齐浏览器请求头，房间、头像与分区封面统一使用稳定磁盘缓存键，避免刷新时间戳造成重复下载和缓存膨胀。
- 分页增加去重、无进展终止和最大请求数，兼容推荐接口固定批量返回，避免重复卡片与无休止补页。

## Windows 高刷新率与交互

- 原生 Runner 读取窗口所在显示器的当前刷新率、同分辨率最高刷新率和支持列表；窗口跨屏或显示模式变化时自动刷新。
- 设置页直接显示当前分辨率和刷新率。本机 3840 × 2400 / 200 Hz 已正确识别，渲染继续跟随 Flutter/Windows 的垂直同步，不强制改动系统显示模式。
- Windows 次级页面使用与当前 Android 更接近的淡入前进转场，滚动统一使用桌面夹持物理模型并在拖动时收起键盘。
- 修复收藏平台监听器在响应式重建中重复注册的问题，减少使用时间增长后的切页和刷新抖动。
- 头像取消批量淡入动画，封面复用稳定缓存，降低高刷新率列表滚动中的无效合成与磁盘请求。

## 验证与构建

- Flutter Analyze 零问题、81 项单元/Widget 测试通过；Bilibili 当前/回退响应、风控错误和图片请求头均加入自动回归。
- 平台接口探测新增 Bilibili 推荐与封面 MIME 校验，26/26 通过；Windows release 已完成本机构建和界面烟雾验证。
- Windows x64 提供可选择安装目录的 EXE 安装包与便携 ZIP，业务数据继续统一保存在安装目录 `AppData`。
