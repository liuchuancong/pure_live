...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `3b09d74b` 已推送。最新红灯实测 WebDAV 默认字号面包屑只有 30.1 px 高、录制设置“打开文件夹”动作只有 21 px 高；现短路径段同时保持至少 48 px 宽高，设置标题动作也保持至少 48 px。WebDAV 与录制设置联合 **37/37**、Analyze 无诊断。此前 `48f79eb7` 已把播放页收藏/录制入口统一为至少 48×48 px，`2ee98324` 已修复 WebDAV 大字号头部高度。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `3b09d74b`，未覆盖安装，也不作为新增布局修复的原生证据 |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前十二项布局修复；手机当前在线且型号/代号/root 已复核，前台仍由其他应用占用，因此只完成只读状态检查，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
