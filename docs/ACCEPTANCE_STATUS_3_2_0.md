...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `07add14f` 已推送。PandaTV 新增[官网 LIVE 与未开播主播双路搜索](PANDALIVE_NATIVE_SEARCH_AUDIT_2026_09_23.md)，受影响测试与只读生产探针 **92/92**，探针合并 13 条（其中未开播 9 条）；本轮全库 Analyze 待源码收敛。此前 17LIVE [官网公开推荐原生游标与当前直播搜索](SEVENTEENLIVE_PUBLIC_CATALOG_SEARCH_AUDIT_2026_09_23.md)定向与生产探针 **6/6**，当批全库 Analyze 超时终止，不计作静态门禁通过；克拉克拉[官网主播关键词分页](KILAKILA_NATIVE_KEYWORD_SEARCH_AUDIT_2026_09_23.md)受影响测试 **195/195**、生产搜索 **1/1**；映客有限精选昵称筛选 **52/52**、生产搜索 **1/1**。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需重跑 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `07add14f`，未覆盖安装；新增平台、搜索、布局及录制等后续修订均待新候选原生验收 |
