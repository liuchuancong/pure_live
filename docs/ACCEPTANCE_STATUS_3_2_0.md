...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `abbc4ea8` 已推送并与 `origin/master` 核对；其后的 `135aa318` 只删除 779 行不可达旧库。最新修复证明关注页固定 44 px 标签筛选条低于 48 px 命中下限，且不足以容纳 3.0 倍文字；现按实际主题、系统缩放和所有可见标签动态计算高度，320×480 / 3.0 倍英文与交互专项 **3/3**、Analyze 无诊断。此前 `71fa2064`、`3ee6b172`、`ef764591` 已修复离线占位页及普通播放页的大字号裁切/溢出。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前八项布局修复；手机当前在线且型号/代号/root 已复核，前台仍由其他应用占用，因此只完成只读状态检查，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
