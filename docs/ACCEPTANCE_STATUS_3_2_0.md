...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `14109535` 已推送。网页搜索房间确认现按最新导航归属，离开或切换直播间后不会打开旧房间；两文件联合回归 **27/27**、定向 Analyze 无诊断，见[专项记录](WEB_SEARCH_NAVIGATION_OWNERSHIP_AUDIT_2026_09_23.md)。上一批 `c4780f34` 已推送。内置网页搜索补齐小红书与 TTing/FLEX 官方直播链接的应用内回流；三文件定向回归 **83/83**、两文件 Analyze 无诊断，见[专项记录](WEB_SEARCH_XHS_TTING_LINK_AUDIT_2026_09_23.md)。上一批 `7c16928a` 已推送。45 个已注册直播站点现均有按稳定身份构造的官网房间目标；最近的 14 站补齐批次定向回归 **125/125**、两文件 Analyze 无诊断，见[专项记录](ROOM_EXTERNAL_OPEN_COVERAGE_AUDIT_2026_09_23.md)。上一批工具箱链接预检见[专项记录](TOOLBOX_ROOM_LINK_PREFILTER_AUDIT_2026_09_23.md)；最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`，下个候选前需重跑 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `14109535`，未覆盖安装，也不作为新增平台、布局修复、斗鱼会话、各平台搜索入口、工具箱链接预检、TwitCasting 精确频道链接、Rumble HLS 修复或 14 站外部打开或小红书/TTing 网页搜索回流或导航后房间确认的原生证据 |
