...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | 最新业务源码 `dba75a5f`：房间详情失败保持待核验，切房清除旧线路，同房元数据重试保留未到期付费消息；见[专项审计](ROOM_DETAIL_ERROR_STATUS_AUDIT_2026_09_24.md)。最近相邻测试 **11/11**，全仓 Analyze 0 error / 0 warning、1 项既有 info（`20260923T170718688Z-quality-focused.json`）。微博现网两段同输入短录复核见[录制证据](WEIBO_RECORDING_SOURCE_RELATIVE_RECHECK_2026_09_24.md)。上一正式 Full 在 `a4f3eeb3` 通过 Flutter **5295/5295**、公共接口 **42/42**（`20260923T153951228Z-quality-full.json`）；该 Full 早于当前业务修复，最终候选仍须重新完成门禁与双端原生验收 |
...
2. `a4f3eeb3` Full 已通过；当前 `dba75a5f` 的相邻测试与全仓 Analyze 已通过。业务源码收敛时完成一次最终 Full，生成一个新 Android arm64 候选。手机进入可用测试窗口后批量收口 A0～A8，不为每个小修复重复构建；设备窗口尚未释放时继续源码或 Windows 工作。
