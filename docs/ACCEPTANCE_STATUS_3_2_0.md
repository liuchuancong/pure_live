...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | 当前业务代码 `4e9895dd`；微博公开推荐快照内昵称筛选的相关测试 **98/98**（`20260923T155843575Z-quality-focused.json`），前一项未知人数修复相关测试 **110/110**（`20260923T154525064Z-quality-focused.json`）。上一正式 Full 在 `a4f3eeb3` 干净输入通过：Analyze 0 error / 0 warning、1 项既有 info，Flutter **5295/5295**，公共接口 **42/42**（`20260923T153951228Z-quality-full.json`）；该 Full 早于当前业务修复，最终候选仍须重新完成门禁与双端原生验收 |
...
2. `a4f3eeb3` Full 已通过，当前 `4e9895dd` 的相关测试已通过；业务源码收敛时完成一次最终 Full，生成一个新 Android arm64 候选。手机进入可用测试窗口后批量收口 A0～A8，不为每个小修复重复构建；设备窗口尚未释放时继续源码或 Windows 工作。
