...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `2ee98324` 已推送并与 `origin/master` 核对。最新修复证明 WebDAV 面包屑固定 50 px 头部在 320×480 / 3.0 倍文字下只给 50 px 高标签留出零垂直余量；现按主题和系统缩放同步计算容器及 Sliver 范围，空/错误态可用高度也扣除真实范围，页面专项 **29/29**、Analyze 无诊断。此前 `abbc4ea8` 已把关注标签筛选条从固定 44 px 改为按可见标签动态增高；`135aa318` 只删除 779 行不可达旧库。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前九项布局修复；手机当前在线且型号/代号/root 已复核，前台仍由其他应用占用，因此只完成只读状态检查，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
