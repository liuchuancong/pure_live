...
| 项目 | 当前证据 |
| --- | --- |
| 源码 | 当前 `a327d959` 已推送并与 `origin/master` 核对；移动端普通控制栏的清晰度/线路弹窗现与全屏选择器一样按实际字号扩展，标题不再横向溢出，选项不再被固定 38 px 行裁切。最近相关五文件 **23/23** 回归、Analyze 无诊断；此前全屏清晰度/线路、全屏换房、本地弹幕样式、房间标签分配和分区卡片大字号修复继续保留。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `a327d959`，未覆盖安装，也不作为新增布局修复的原生证据 |
