...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `71fa2064` 已推送并与 `origin/master` 核对；其后的文档/流程提交不改变应用二进制。最新修复以真实离线播放占位页证明 320×480 / 3.0 倍英文下固定标题栏把 63 px 文字裁到 31 px，正文向下溢出 353 px；现按主题和系统缩放增高标题栏，正文在空间不足时滚动，相关四文件 **34/34**、Analyze 无诊断。此前 `3ee6b172` / `ef764591` 已修复普通播放页观看人数、清晰度和线路行的 641 px 横向溢出及 48→31 px 纵向裁切，并删除 313 行死代码和 173 行无效测试。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `71fa2064`，未覆盖安装，也不作为新增布局修复的原生证据 |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前七项布局修复；手机两次检查均由其他应用占用，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
