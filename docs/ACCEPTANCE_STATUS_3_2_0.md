...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `5644b45e` 已推送。最新红灯证明全屏换房标题行仅 36 px、清晰度/线路标题行仅 35 px，无法容纳 48 px 的刷新/关闭动作；全屏本地弹幕输入行也固定为 38 px。现三处操作均至少 48 px，竖屏双行控制栏由 104 px 调整为 112 px 并同步提示避让；相关四文件 **32/32**、Analyze 无诊断。此前 `3b09d74b` 已修复 WebDAV 面包屑及录制设置标题动作，`48f79eb7` 已修复播放页收藏/录制入口。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前十五项布局修复；手机当前在线且型号/代号/root 已复核，前台仍由其他应用占用，因此只完成只读状态检查，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
