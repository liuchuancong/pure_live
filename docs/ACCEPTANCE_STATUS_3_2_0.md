...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | `3ee6b172` 已推送并与 `origin/master` 核对；其后的文档/流程提交不改变应用二进制。真实普通播放页的观看人数、清晰度和线路行在 320×480 / 3.0 倍英文下由旧实现横向溢出 641 px，现改为有界弹性分配，长标签单行省略且完整菜单仍可选择。审查同时删除 313 行无生产调用点的旧选择器实现及其 173 行专用测试，避免继续维护死代码；相关七文件 **31/31** 回归、Analyze 无诊断。此前五项大字号布局修复继续保留。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `3ee6b172`，未覆盖安装，也不作为新增布局修复的原生证据 |
