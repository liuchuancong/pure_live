...
| 项目 | 当前证据 |
| --- | --- |
| 源码 | 当前 `45e0bb4f` 已推送并与 `origin/master` 核对；全屏清晰度/线路选择器按实际字号扩展固定行，极矮窗口按真实可用区分配；全屏换房面板的标题、页签、卡片详情和封面覆盖层也按文字缩放扩展，大字号页签改为横向滚动而非缩小。前者与切流事务联合 **46/46**，后者最终布局回归 **19/19**，Analyze 无诊断；分区卡片 3.0 倍文字修复继续保留。最近一次完整门禁仍绑定 `b79e8838`：Flutter **4999/4999**、公共接口 **42/42**，记录 `20260919T181710597Z-quality-full.json`；下个候选前需在源码批次收敛后重跑一次 Full |
| Android 最新本机构建/原生输入 | 现存 `b79e8838` arm64 Debug，3.1.8+4121 / Manifest 6121，289007467 B，SHA-256 `377D4CD5…30ED`；16 个原生库、最小 ELF LOAD `0x4000`、APK 内容门禁通过。该包早于当前源码 `45e0bb4f`，未覆盖安装，也不作为新增布局修复的原生证据 |
...
1. **当前原生闭环缺失**：Android 现存 `b79e8838` 候选已通过当时完整门禁，但早于当前三项布局修复；手机两次检查均由其他应用占用，未安装或发送界面输入。Windows 最新原生证据同样早于当前 `master`。
