...
| 项目 | 当前证据 |
| --- | --- |
| 功能源码基线 | 当前 `a4f3eeb3`（业务代码为 `9a15c7fe`），干净输入的正式 Full **通过**：全仓 Analyze 0 error / 0 warning、1 项既有 info，Flutter **5295/5295**，公共接口 **42/42**；记录 `20260923T153951228Z-quality-full.json`，运行中源码未变化。仍需双端当前候选原生验收；后续业务代码变更须重新验证其影响范围与最终门禁 |
...
2. 当前 `a4f3eeb3` Full 已通过；后续业务源码收敛时按变更范围重验，生成一个新 Android arm64 候选。手机进入可用测试窗口后批量收口 A0～A8，不为每个小修复重复构建；设备窗口尚未释放时继续源码或 Windows 工作。
