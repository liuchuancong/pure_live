...
- 真实进入虎牙 `狂鸟丶楚河-90327` 房间，当前画质为“蓝光30M”、线路为“线路1”；画面、声音、覆盖弹幕和下方弹幕列表均开始工作，Android 媒体会话报告 `PLAYING`。
- 切换到其他前台应用后连续采样 10 分钟，共 21 个 30 秒间隔样本；所有样本均保持 `PLAYING`，Pure Live 没有抢回前台。
- PSS 为 438,712～497,718 KB，平均 475,296.7 KB；线性拟合斜率约 `+283.9 KB/min`。RSS 为 643,384～702,020 KB，平均 680,042.3 KB；拟合斜率约 `+299.3 KB/min`。该时窗内呈波动平台而非持续陡增。
- CPU 瞬时样本平均 2.24%、最高 5%。厂商媒体日志显示后台普通视频仍在解码但画面帧被丢弃；这是保持视频模式快速回前台的当前策略，不把它误报为内存泄漏。需要最低功耗时应使用现有纯音频/ASMR 模式，普通视频模式的功耗取舍继续保留为产品策略测试项。
- 测试结束后强制结束本轮 Pure Live 测试进程；60 秒核对时进程、媒体会话、应用后台播放 Wake Lock 和 AudioService Wake Lock 均不存在，当前前台应用未受影响。
- 证据：`huya-background-10min-20260831.csv`、`huya-background-10min-summary-20260831.json`、`huya-background-release-summary-20260831.json`。

## 当前结论与边界

- 本轮直接通过：覆盖升级与数据保留、10 次冷启动、分类/搜索标签硬边界、CC 分类迁移回退、三档刷新率与持久化、Twitch 经可达 Clash 应用代理的原生搜索，以及虎牙普通视频后台播放 10 分钟与结束后资源释放。
