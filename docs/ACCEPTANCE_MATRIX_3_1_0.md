...
| ID | 状态 | 验收内容 |
|---|---|---|
| A6-01 | RUN | K90 Pro 的 Bilibili 实录中，录制中心已显示 35 秒、2.75 MB、1.1x，停止后状态准确；开始前目录探测、准备态和多点单调序列继续补证 |
| A6-02 | RUN | Bilibili 停止、封装、已停止状态和取消监控通过；失败重连、待开播、离线恢复、分组及滚动边界继续执行 |
| A6-03 | RUN | Bilibili 成品为 7,763,631 B、60.086333 秒，`ffprobe` 读到 H.264 + AAC，私有目录文件可实读；其他平台矩阵继续执行 |
| A6-04 | RUN | 当前 Bilibili 样本停止后监控移除，强制停止后进程消失且活动 Wake Lock 无 Pure Live；签名/线路过期续接、跨分片累计与正常退出资源释放继续执行 |
