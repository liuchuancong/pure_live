...
| A1-05 | RUN | 历史、标签、工具箱、IPTV、WebDAV、备份/恢复、关于与更新检查的源码/Widget 合同已覆盖加载/空/错误态、窄屏大字号、滚动、路由退出、重复操作、迟到结果和对象身份；早期原生五场景见 [辅助页面审查](AUXILIARY_PAGES_AUDIT_2026_09_07.md)。09-11～09-15 的分项修订各有独立审计，其中版本下载、账号退出、缓存/历史清理、标签管理及二维码同步均已加入单次事务和响应式动作。`9f115f6a` 进一步修复房间卡片标签分配在 1.9 倍文字下固定 68 px 行溢出 21 px，相关三文件 16/16、Analyze 无诊断。真实 WebDAV 深层目录与传输、系统下载/安装、双端候选触摸/键盘/系统返回及重启数据保留继续；代表证据见 [WebDAV 审计](WEBDAV_STATE_LAYOUT_AND_DIRECTORY_PATH_AUDIT_2026_09_11.md)、[工具箱审计](TOOLBOX_ROUTE_AND_SELECTOR_LAYOUT_AUDIT_2026_09_12.md)、[IPTV 管理审计](IPTV_MANAGE_LAYOUT_AND_LIFECYCLE_AUDIT_2026_09_12.md)和[标签管理审计](TAG_MANAGEMENT_DETAIL_ACCESSIBILITY_AND_ROUTE_AUDIT_2026_09_15.md)。 |
...
| A3-04 | RUN | cycle 208 在抖音竖屏源的横屏全屏中发送 Android 系统返回，先恢复 `1200×2608` 普通直播页且竖屏手势与弹幕栏仍存活；竖屏沉浸的控制栏现直接接管上滑恢复，不再依赖控制栏先隐藏。09-12 当前直播间定时器改为本地草稿和单次提交，取消不改变计时会话，停用、数字边界、退出生命周期与 320×480 / 3.0 倍英文布局新增 5 项回归，见 `docs/ROOM_PLAYBACK_TIMER_TRANSACTION_AND_LAYOUT_AUDIT_2026_09_12.md`。同日房间音量改为自有草稿和等待式提交，提交期系统返回保持路由，失败保留原设置；非法持久化边界和窄屏大字号布局新增 9 项回归，见 `docs/ROOM_VOLUME_DIALOG_TRANSACTION_AND_LAYOUT_AUDIT_2026_09_12.md`。09-20 `2fac1480` 将全屏清晰度/线路面板的标题、分组和选项高度绑定实际主题字号与系统缩放，并在极矮窗口内按真实可用高度分配两组可滚动内容；1.0 倍既有几何、3.0 倍文字和切流事务联合 46/46。随后 `45e0bb4f` 将全屏换房面板的标题、页签、卡片详情和封面覆盖层纳入同一文字度量，大字号页签横向滚动、卡片网格纵向滚动；原固定 36 px 详情区的 3.0 倍文字红灯为底部溢出 0.27 px，修订后布局 19/19、Analyze 无诊断。当前源码尚待候选原生复验，其他对话框/底部面板的返回优先级及连续第二次返回仍按本行继续 |
| A3-05 | RUN | `tool/android_recording_smoke.ps1 -ExerciseStreamSelection` 现把“仅打开菜单”提升为真实选择、提交后稳定及错误态门禁。K90 Pro / cycle 212～216 已在最终 Debug APK 分别完成虎牙 `蓝光30M→流畅`/`线路1→线路2`、斗鱼 `原画1080P60→超清`、快手 `蓝光 质臻→超清`、Bilibili `线路1→线路2`、抖音 `高清→标清`/`线路1→线路2`，每次切换后继续稳定播放并完成 H.264 + AAC 短录；单一档位/线路按平台实际能力记为不适用。确定性回归另覆盖稳定平台 ID、服务端实际档位回写、相同 URL 拒绝假切换、快速点击 latest-wins、失败原子回滚和新线路数钳制；Android 原生播放器尚不具备 Windows 离屏首帧接管能力，网易 CC、Twitch、SOOP、YY 的实际切换继续逐项采样。09-20 `3ee6b172` 修复真实普通播放页观看人数/清晰度/线路行：320×480 / 3.0 倍英文下旧行横向溢出 641 px，现按可用宽度弹性分配并以单行省略保留完整菜单操作；同时删除 313 行已失去生产调用点的旧选择器及 173 行只覆盖死代码的测试，相关七文件 31/31、Analyze 无诊断。首个证据 `local-artifacts/diagnostics/android-recording-smoke-20260905T044047480/summary.json`，其余逐轮证据见 Android 审计 |
