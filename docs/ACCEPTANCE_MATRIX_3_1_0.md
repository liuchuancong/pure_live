...
| 虎牙 | PASS（热门进房） | PASS（当前房间） | RUN（当前卡片） | RUN（蓝光30M/线路1，待切换） | RUN（待消息断言） | PASS（当前样本） | PASS（当前短录） |
...
| ID | 状态 | 验收内容 |
|---|---|---|
| A6-01 | RUN | Bilibili 录制中心显示 35 秒、2.75 MB、1.1x；虎牙显示 40 秒、19.00 MB、1.2x、4.2 Mbps，且私有 TS 双采样从 20,709,376 B 增至 21,495,808 B。开始前目录探测和准备态继续补证 |
| A6-02 | RUN | Bilibili、虎牙的停止、封装、已停止状态和取消监控通过；失败重连、待开播、离线恢复、分组及滚动边界继续执行 |
| A6-03 | RUN | Bilibili 成品 7,763,631 B / 60.086333 秒；虎牙成品 27,681,159 B / 56.813667 秒、H.264 2560×1440@120 + AAC；其他平台矩阵继续执行 |
| A6-04 | RUN | Bilibili、虎牙样本停止后监控均移除，强制停止后进程消失且活动 Wake Lock 无 Pure Live；签名/线路过期续接、跨分片累计与正常退出资源释放继续执行 |
