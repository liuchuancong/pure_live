...
| A1-05 | RUN | 历史、标签、工具箱、IPTV、WebDAV、备份/恢复、关于与更新检查的源码/Widget 合同已覆盖加载/空/错误态、窄屏大字号、滚动、路由退出、重复操作、迟到结果和对象身份；早期原生五场景见 [辅助页面审查](AUXILIARY_PAGES_AUDIT_2026_09_07.md)。09-11～09-15 的分项修订各有独立审计，其中版本下载、账号退出、缓存/历史清理、标签管理及二维码同步均已加入单次事务和响应式动作。`9f115f6a` 修复房间卡片标签分配在 1.9 倍文字下固定 68 px 行溢出 21 px；09-20 `2ee98324` 又修复 WebDAV 面包屑固定 50 px 头部在 3.0 倍文字下没有垂直余量的问题，容器、Sliver 与空/错误态可用高度现共享真实文字范围，页面 29/29、Analyze 无诊断。真实 WebDAV 深层目录与传输、系统下载/安装、双端候选触摸/键盘/系统返回及重启数据保留继续；代表证据见 [WebDAV 审计](WEBDAV_STATE_LAYOUT_AND_DIRECTORY_PATH_AUDIT_2026_09_11.md)、[工具箱审计](TOOLBOX_ROUTE_AND_SELECTOR_LAYOUT_AUDIT_2026_09_12.md)、[IPTV 管理审计](IPTV_MANAGE_LAYOUT_AND_LIFECYCLE_AUDIT_2026_09_12.md)和[标签管理审计](TAG_MANAGEMENT_DETAIL_ACCESSIBILITY_AND_ROUTE_AUDIT_2026_09_15.md)。 |
