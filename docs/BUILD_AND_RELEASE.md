...
当前发布候选：2026-08-23，v2.9.0 build 4079，Windows 11 + Java 25 + Flutter 3.47.0。完整门禁在源码冻结后只执行一次，并把 Analyze、测试、公开接口探测及各平台产物结果写入 `STAGE_UPDATE_2_9_0.md`。Android arm64、Windows x64、Linux x64、macOS universal 和 iOS arm64 按独立阶段串行构建。干净便携目录继续把数据、缓存和临时文件写入 release 同级 `AppData`。
