...
- **09-24 微博现网录制闭环**：承接下方同日元数据抽样，`f0945358` 的注册适配器经真实目录→详情→媒体续取、生产 FLV relay/FFmpegKit、手动排空和显式调用生产 MP4 收尾通过 **1/1**。目标 12 媒体秒/至少两段达到；成品 3,037,726 B、2 段来源，完整严格解码通过。留存的同次 FLV 输入与成品按解码帧顺序比较，视频 256/256、音频 276/276 内容一致；音频相对时间偏移跨度约 1.13 ms。终态 code=0、inputDrained=true，强制取消、尾片丢弃、活动缺片和包损坏标记均为 false。证据为 `local-artifacts/build-records/20260924T101247401Z-quality-focused.json` 与本机 `local-artifacts/weibo-1790244658046630/summary.json`；源 URL/媒体不入库。此为 Windows 本机适配器至录制成品证据，仍需当前 GUI 候选、Android、长录、续期与资源矩阵；不提升历史整组状态。

- **09-24 当前候选在线抽样**：`faa21136` 的公开接口探针重新通过 **42/42**。AcFun 真实目录返回 2 间、目标详情给出 4 档；蓝光 8M 与高清两档各连续读取约 8 秒，均为有效 FLV 前缀，播放恢复重新取得 URL，见 `local-artifacts/build-records/20260924T094735395Z-quality-focused.json`。微博真实目录有 9 张卡，搜索快照回流至同一场次，详情确认在播与主播/场次身份并声明 1 个媒体源；本轮未读取该媒体。小红书两个既有精确房间样本均明确下播、媒体列表为空，证明离线映射但不证明在播取流。后两项记录为 `local-artifacts/build-records/20260924T095129001Z-quality-focused.json`。这些均为适配器/传输证据，不替代 Android/Windows 画面、声音、录制及资源释放验收，历史 42 组 RUN 保持。

- **09-23 两个待注册平台再核对**：本机 Clash `127.0.0.1:7897` 指定代理下，浪 Live 公开 `langweb/v1/room/liveinfo` 仍返回 CloudFront 403；战旗官网目录 API 返回 HTTP 200、五张 `status=4` 卡片。本轮只取公开元数据，未取得新的有效 FLV/HLS 前缀，两个内部适配器继续保持未注册，见[外部打开覆盖与门槛记录](ROOM_EXTERNAL_OPEN_COVERAGE_AUDIT_2026_09_23.md)。

- **09-22 当前差距：LOOK 直播已应用注册**：[合同检查点](LOOK_LIVE_CONTRACT_CHECKPOINT_2026_09_22.md)。视频/语音推荐、精确房间与当前页关键词筛选、状态、指标分列、HLS/FLV、客户端专用房型提示和播放/录制恢复已接通。3.2.8 起当前 **34 个直播站点 + IPTV，0 组未注册**：下线 11 个难维护平台，并删除未注册的战旗、浪 Live 代码（见[平台兼容性](PLATFORM_COMPATIBILITY.md)开头）。DLive、一直播与企鹅电竞已归档生命周期证据，Trovo 继续核对服务状态；下方旧状态为取证历史。完整平台能力、双端验收和 3.2.0 发布继续，未缩减目标。
