...
- 对象：同一 v3.1.7 Windows x64 便携产物，独立实例 `--instance=ui317b_20260901T083000`。
- 虎牙热门页在 8 秒观察窗内完成 20 张卡片及缩略图加载，首屏热度按 `131.7万、97.2万、83.0万……` 递减。
- 打开 `小飒电竞丶董小飒` 房间后取得实时画面，弹幕服务器连接正常，列表持续追加完整昵称弹幕。
- 画质菜单真实返回 `蓝光20M / 蓝光8M / 蓝光4M / 超清 / 流畅`；从 `蓝光20M` 切到 `蓝光8M` 后顶部状态更新，画面和弹幕继续。
- 线路菜单真实返回 `线路1～线路4`；从 `线路1` 切到 `线路2` 后顶部状态更新，画面和弹幕继续。
- 录制使用录制设置中的独立默认档位，而不是强行继承当前播放器档位；因此播放器为 `蓝光8M / 线路2` 时，录制中心显示录制器实际应用的 `蓝光20M / 线路1`，与 `StreamResolverService` 的当前合同一致。
- 录制中心实时显示时长、累计大小、速度和码率；停止时为 `00:03:18 / 82.75 MB / 1.1x / 2.1 Mbps`，状态从“录制中”切换为“已停止”，后台 FFmpeg 进程归零。
- 虎牙短签名在本次会话中轮换一次，最终得到两个 MP4；两段均可由 `ffprobe` 实读：

| 文件 | 时长 | 大小 | 视频 | 音频 |
|---|---:|---:|---|---|
| `20260901_084224_483.mp4` | 104.968667 秒 | 62,650,135 bytes | H.264 1920×1080 60 fps | AAC 44.1 kHz 双声道 |
| `20260901_084407_901.mp4` | 90.581667 秒 | 20,488,637 bytes | H.264 1920×1080 60 fps | AAC 44.1 kHz 双声道 |

两段媒体时长合计 195.550334 秒，接近录制中心累计的 198 秒；差值位于连接/封装切换边界，没有出现空文件或不可解码文件。

### 7.1 实测发现并修复的会话时间错误

v3.1.7 运行包在短签名续接后把录制中心左下角时间从首段 `08:42` 改成第二段 `08:44`。根因是 `LiveRecordTask.createTime` 同时承担“用户录制会话开始时间”和“本次 FFmpeg 尝试文件前缀”两种语义；`beginNewAttempt()` 为避免文件名冲突必须刷新前缀，却连带覆盖了用户可见的会话时间。

当前工作树加入独立持久化字段 `recordingStartedAt`：

1. 用户明确启动/重新启动录制时写入一次；
2. CDN 续签、换线或自动重连只更新尝试前缀，不覆盖会话时间；
3. 旧 schema 没有该字段时回退到原 `createTime`，避免迁移崩溃；
4. schema 升到 6，重启恢复后仍显示原会话开始时间。

聚焦持久化与重试回归 13/13 通过：`local-artifacts/build-records/20260901T011808521Z-quality-focused.json`。该修复尚未进入 v3.1.7 二进制，下一安装包需用相同“首段→续签→停止→重启恢复”链复验。
