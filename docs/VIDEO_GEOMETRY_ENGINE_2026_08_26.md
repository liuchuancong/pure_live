...
竖屏节目进入横屏全屏时，呈现层固定使用 `BoxFit.contain`，播放器未使用区域的 `fill` 设为透明；模糊封面属于播放器后方的环境层。`fit` 与 `fill` 必须同时传到 MediaKit、Fijk 或 BetterPlayer 的原生 Widget，单独在 Flutter 外层放置背景仍会被播放器默认黑色画布遮住。普通页和小窗继续由各自容器策略决定，不写回用户的全局画面适配设置。
