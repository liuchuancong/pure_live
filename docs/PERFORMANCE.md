...
Pure Live 在 Android 上提供省电、均衡、高性能三档刷新率。新安装默认省电并交由系统动态选择；均衡只在触摸、滚动和转场期间请求当前设备最高刷新率；高性能在应用前台持续请求最高刷新率。原生层监听显示器模式变化，折叠屏切换、外接屏变化、应用恢复前台或系统调整显示模式后都会重新检测并回传当前/最高 Hz。设置入口为“设置 → 通用 → 界面刷新率”。
...
- 应用 UI 按省电/均衡/高性能策略请求刷新率；自动弹幕渲染主画面限制为 60 FPS、小窗限制为 30 FPS，手动模式最高 240 FPS。`flame_barrage` 只在存在等待或活动弹幕时启动逻辑脉冲，空闲、暂停、清屏或后台阶段均停止调度；所有轨道严格采用同一 px/s 速度。
