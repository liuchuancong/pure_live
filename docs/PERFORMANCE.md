...
本轮 Android 16 / 120 Hz 设备实测：`mActiveRenderFrameRate=120.00001`，系统记录 `AppRequestRefreshRates=120 Hz`；覆盖安装后的 5 次冷启动 `TotalTime` 为 239–274 ms，均保持前台可见且崩溃缓冲区为空。横竖屏切换后视频弹幕与弹幕列表继续更新；同一 PID 内完成前台长时间音频、切回视频、后台停用视频轨和回前台预热恢复，无崩溃、ANR 或恢复超时。
