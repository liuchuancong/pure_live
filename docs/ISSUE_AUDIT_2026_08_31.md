...
审计基线：维护分支 `v3.1.0+4113` / `b1182034`，当前版本 `v3.1.8+4121`。本轮只读取上游 Issue 和当前维护分支代码进行归因，不合并上游提交。状态只描述已经取得的证据；静态覆盖不会替代 Android / Windows 实机结果。

快照时间为 2026-09-02（Asia/Shanghai）：维护仓库当前没有未关闭 Issue；上游未关闭列表共 14 项，编号为 #833、#832、#830、#829、#828、#827、#825、#824、#821、#819、#792、#779、#767、#708，最新项为 #833。#801、#807、#810、#817、#818、#820、#823、#826 已由上游关闭；关闭状态只表示 Issue 生命周期变化，不替代本维护分支的代码与运行验证。下表覆盖全部当前未关闭上游 Issue，并保留与维护分支直接相关的已关闭项、闭环或后续设计记录。

## 结论表

| Issue | 类型与归因 | 当前证据 | 处理状态 |
|---|---|---|---|
| [#833 Windows 播放画面出现在应用外](https://github.com/liuchuancong/pure_live/issues/833) | 旧版运行异常；报告基于 v3.0.9，只有两张截图，没有直播平台、窗口/DPI、播放器日志或进程信息 | 当前 v3.1.8 Windows x64 正式便携实例已真实打开 Bilibili 房间，视频持续嵌在 Pure Live 窗口内，本地/远端弹幕、设置和短录可用；退出后应用与 FFmpeg 残留进程均为 0。证据见 `docs/WINDOWS_RUNTIME_AUDIT_3_1_8.md` | 当前维护版本没有稳定复现，现有代码不按旧截图回退播放器。把“普通窗口/最大化/真全屏/PiP/多窗口 × 100%～200% DPI”继续留在 Windows 矩阵；若当前版本再次出现，采集 HWND 树、窗口矩形、显示器 DPI 和 media_kit 日志后再归因 |
| [#832 锁屏或不进房录制](https://github.com/liuchuancong/pure_live/issues/832) | 两项诉求混在一条 Issue：锁屏后继续现有录制是稳定性验收；从录制中心直接选择房间并创建任务是新增入口 | K90 Pro / Android 17 已强制进入 Dozing/锁屏 60 秒，期间同一私有 TS 从 1,310,720 B 增至 7,602,176 B，进程和媒体前台服务存活；唤醒后房间恢复，停止得到 11,224,519 B、112.749333 秒、H.264 + AAC MP4，全部 25 项断言通过。证据为 `local-artifacts/diagnostics/android-recording-smoke-20260902T004408576/summary.json` | 当前锁屏短样本通过；仍保留 10 分钟厂商省电、断网恢复和低电量长时门禁。现有 `RecorderController.addTask` 需要完整 `LiveRoom`，录制中心只能操作已创建任务，因此“不进直播页首次选房创建任务”按独立功能设计，不与锁屏稳定性混成播放器缺陷 |
| [#830 macOS 3.0.9 启动崩溃](https://github.com/liuchuancong/pure_live/issues/830) | `community-platform`；报告给出 M5 Mac、v3.0.9、v3.0.8 最后正常和完整原生崩溃栈，证据指向 macOS `media_kit` 硬件纹理初始化，不是首页刷新业务 | 栈在 `TextureHW.getProcAddress → CFBundleGetBundleWithIdentifier` 读取异常对象时崩溃，随后才进入 mpv render context；Android/Windows 播放、弹幕与首页代码不经过这条 macOS 原生路径 | 本仓库继续以 Android/Windows 为发布门禁。保留崩溃栈作为社区平台证据，后续核对 v3.0.8→v3.0.9 的 macOS 插件、framework 打包和签名差异；没有 macOS 构建与 M5 运行证据前，不改公共播放器生命周期来掩盖原生 TextureHW 崩溃 |
| [#829 本地互动入口关闭后仍遮挡画面](https://github.com/liuchuancong/pure_live/issues/829) | `fork-regression`；设置页描述与竖屏列表已按开关隐藏，但横屏及竖屏全屏的独立 composer 在关闭状态仍主动渲染“启用本地互动”长条 | `FullscreenLocalDanmakuComposer` 的 disabled 分支直接创建 `FilledButton.tonalIcon`，因此设置关闭仍占据全屏底部中央命中区；普通竖屏列表和菜单入口已有 `enabled` 守卫，根因不在配置持久化 | 全屏 composer 已完全服从全局开关：关闭时返回零尺寸，开启时保留输入框。导航/UI 聚焦回归 11/11、Analyze 0；K90 Pro 实机将开关设为关闭后进入 2608×1200 横屏全屏，提示长条确实消失，最后恢复原设置，证据为 `local-artifacts/diagnostics/android-local-interaction-20260901T154001015/summary.json` |
...
| [#823 弹幕服务器首次失败后持续重连](https://github.com/liuchuancong/pure_live/issues/823) | 已关闭的共享传输缺陷；旧周期定时器和会话所有权可能在成功后继续建并行连接，或在最终失败后仍把死连接当作当前房间 | 当前 `WebScoketUtils` 使用单次定时重试、端点轮换、连接代际、90 秒以上无消息 watchdog 和最多 8 次有界重连；成功后取消重连定时器，手动关闭推进代际并释放 subscription/socket。`DanmakuController` 对暂态“正在尝试重连”保留会话，最终失败清除 room key，使手动恢复能创建新传输 | 2026-09-01 聚焦执行 WebSocket、弹幕生命周期、虎牙协议和多画面退出 4 个文件共 17/17 通过；记录 `local-artifacts/build-records/20260901T031920369Z-quality-focused.json`。真实网络断开、端点轮换和 UI 状态仍在 Android/Windows 实播矩阵观察，不把 fake channel 测试外推为全部平台网络通过 |
