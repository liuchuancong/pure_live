...
审计基线：维护分支 `v3.1.0+4113` / `b1182034`，当前目标补丁版本 `v3.1.3+4116`。本轮只读取上游 Issue 和当前维护分支代码进行归因，不合并上游提交。状态只描述已经取得的证据；静态覆盖不会替代 Android / Windows 实机结果。

快照时间为 2026-08-31（Asia/Shanghai）：维护仓库当前没有未关闭 Issue；上游未关闭列表共 9 项，最新项为 #824。#801、#807、#810、#818 已由上游在 2026-08-31 关闭；关闭状态只表示 Issue 生命周期变化，不替代本维护分支的代码与运行验证。下表覆盖全部当前未关闭上游 Issue，并保留这四项与维护分支直接相关的闭环或后续设计记录。
