...
审计基线：维护分支 `v3.1.0+4113` / `b1182034`，当前发布基线 `v3.1.8+4121` / `e94f94d7`。本轮只读取上游 Issue 和当前维护分支代码进行归因，不合并上游提交。状态只描述已经取得的证据；静态覆盖不会替代 Android / Windows 实机结果。

快照时间为 2026-09-01（Asia/Shanghai）：维护仓库当前没有未关闭 Issue；上游未关闭列表共 12 项，编号为 #830、#829、#828、#827、#825、#824、#821、#819、#792、#779、#767、#708，最新项为 #830。#801、#807、#810、#817、#818、#820、#823、#826 已由上游关闭；关闭状态只表示 Issue 生命周期变化，不替代本维护分支的代码与运行验证。下表覆盖全部当前未关闭上游 Issue，并保留与维护分支直接相关的已关闭项、闭环或后续设计记录。
...
| [#829 本地互动入口关闭后仍遮挡画面](https://github.com/liuchuancong/pure_live/issues/829) | `fork-regression`；设置页描述与竖屏列表已按开关隐藏，但横屏及竖屏全屏的独立 composer 在关闭状态仍主动渲染“启用本地互动”长条 | `FullscreenLocalDanmakuComposer` 的 disabled 分支直接创建 `FilledButton.tonalIcon`，因此设置关闭仍占据全屏底部中央命中区；普通竖屏列表和菜单入口已有 `enabled` 守卫，根因不在配置持久化 | 当前工作树把全屏 composer 改为完全服从全局开关：关闭时返回零尺寸，开启时保留输入框；用纯判定合同锁定横屏与竖屏全屏一致性。导航/UI 聚焦回归 11/11、Analyze 0 通过；当前实机仍安装 v3.1.8 发布包，修复后二进制的开关开/关与横竖屏观察留到 Debug APK 的 C 轮次 |
