...
- Android 本地构建按当前 `media_kit/hook/native_bundles.json` 预取并校验四个 ABI 的 Native Assets；质量门禁和 Windows 构建预取 media_kit 与 FFmpeg 的 Windows 档案。Android/Windows 的 FFmpeg 底层已从上游 builders v0.11.1 自带的 9.0.1 重编译至官方 `n9.0.2`，继续使用最新 Flutter 钩子包 `ffmpeg_kit_extended_flutter 0.6.2`。项目的[原生依赖资产预发布](https://github.com/wzgrx/pure_live/releases/tag/native-ffmpeg-9.0.2-b1)固定 Android 三 ABI AAR SHA-256 `c6c9b1ff7be756b0fb587f98e05972ca4dae97e8275c22961b443b4fd5f49bf7`、Windows x86_64 ZIP SHA-256 `e61684a91f7471ba00f1d5b36a24e93ab602ef72bd57000d94990e7e0c5dfe3a`。本机构建先核对 SHA-256，再写入按原生版本隔离的 Native Assets 共享缓存；AAR 合并脚本 `tool/assemble_ffmpeg_android_aar.py` 同时核对 ELF 架构、版本标记和页面对齐。官方远端构建通过 `pubspec.yaml` 的平台资产 URL 使用同一版本，构建后须继续核验实际 APK/DLL。Linux、macOS、iOS 当前仍沿用上游原生包，继续分别迁移和验证。
