...
| 批次 | 平台 | 源码状态 | 下一步 |
| --- | --- | --- | --- |
| A | 已注册 32 个直播站点 + IPTV | 应用入口、能力表与设置目录已覆盖 | 补齐各平台双端原生与录制证据 |
...
| C | DLive | 当前 Streamlink 仍维护签名取流实现，列为下一生产合同批次 | 核对公开目录 GraphQL、频道身份、签名 HLS 与观看人数 |
| C | VK Video Live | 当前 Streamlink 维护 `vkvideolive` 插件，列为下一生产合同批次 | 核对目录、搜索、频道/场次身份及 HLS/DASH |
