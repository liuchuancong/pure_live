...
- K90 Pro 在 10 分钟后自动锁屏且没有密码。Pure Live 包装器会在取得 C 轮租约、执行任何真实设备命令之前统一调用 `tool/wake_android_device.ps1`：优先选择唯一 IPv4 ADB transport，发送唤醒和 `dismiss-keyguard`，复核系统 Keyguard；只有系统仍明确报告锁屏时才执行按当前分辨率计算的上滑。具体 UI/运行冒烟保留同样的二次复核，因此长队列等待后也不会把锁屏页当成应用界面。
