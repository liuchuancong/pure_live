...
逻辑优先级固定为 `A → B → C → A`。同一时刻只有一个 lane 持有文件租约；一轮命令结束后自动记录结果并交给下一 lane。三个任务都已登记等待时会严格逐个执行，不会并发触摸、安装或重启同一部手机上的应用。预期 lane 在 120 秒内没有登记时，协调器才选择后续已经等待的 lane，避免一个已停止的任务永久锁住整条队列；要保持完整 A/B/C 循环但本轮没有实机步骤，当前 lane 应提交一个显式空轮次。
