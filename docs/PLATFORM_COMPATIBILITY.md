...
| GoodGame | 官网 API 的 GoodGame 原生直播目录，保留每页 50 条和服务端总量 | 精确频道/播放器链接；当前目录页内按标题、主播和游戏匹配 | [官网 Chat v2 游客只读 WebSocket](GOODGAME_PUBLIC_CHAT_AUDIT_2026_09_23.md)，按数字频道 ID 加入并接收当前消息；双端原生消息验收待执行 | `viewers` 为当前观看；`rating`、premium 计数与粉丝分别处理 |
