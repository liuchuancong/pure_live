...
Picarto 已进入 Android 候选并取得部分原生证据，见 [接入审计](PICARTO_ADAPTER_AUDIT_2026_09_07.md)及[停止/清理补证](ANDROID_PROXY_OCCLUSION_AUDIT_2026_09_07.md)。当前源码新增官网动态直播分类与原生分类分页，见[分类专项记录](PICARTO_NATIVE_CATEGORY_AUDIT_2026_09_23.md)；这项更新尚未进入原生候选。
...
| Picarto | 官网动态直播分类、公开目录与分类原生分页；每次读取当期分类元数据 | 官网搜索入口，分享链接回流 | 当前未接入，页面明确说明 | `viewers` 为在线，详情 `total_views` 为累计观看；分类 `online_channels` 不当作观众数 |
