...
- 上游基线：`liuchuancong/pure_live@ff8c0469`
- 发布日期：2026-08-25

## 本轮范围

- 通过连续真实 merge 把上游从 `974f4c32` 同步到发布冻结点 `ff8c0469`，保留共同祖先和维护分支历史。
...
- 发布冻结源码 `2a1ffbb8` 的 Flutter Analyze 结果为 `No issues found`；完整 Flutter 测试共 `387/387` 通过，覆盖页面刷新、播放器/音频模式、PiP、弹幕生命周期、画质事务、录制配置持久化、搜索、热度/在线人数和 Windows 窗口状态。
- 同一门禁的公开接口合同 `42/42` 通过，包含抖音搜索瞬态重试、YY #798 匿名 HLS及斗鱼 #799 房间 `71415` 的元数据、H5 清晰度/CDN与实际 FLV 文件头。
- 完整记录：`local-artifacts/build-records/20260825T093258451Z-quality-full.json`；总用时 553.173 秒，峰值 CPU 32.53%，峰值工作集 5,613,486,080 字节，结束后活跃重型进程为 0。
