...
class AppSettingsController extends GetxController {
  ...
  late final RxString refreshRateModeName = hiveString(
    'refreshRateMode',
    _initialRefreshRateMode(),
  );
  final RxBool preferRealOnlineCounts = hiveBool('preferRealOnlineCounts', false);
  late final RxList<String> realOnlinePlatforms = hiveStringList(
    'realOnlinePlatforms',
    defaultRealOnlinePlatforms,
  );
  ...
  late final RxList<String> savedMenuIds = hiveStringList(
    'savedMenuIds',
  ...
  bool isRealOnlineEnabledFor(String? platform) =>
      realOnlinePlatforms.contains(platform?.trim().toLowerCase());
  ...
  void fromJson(Map<String, dynamic> json) {
    ...
    asmrSleepMinutes.v = (((json['asmrSleepMinutes'] as num?)?.toInt() ?? 60).clamp(
      1,
      maxSleepMinutes,
    )).toInt();
    ...
    realOnlinePlatforms.v = List<String>.from(
      json['realOnlinePlatforms'] ?? defaultRealOnlinePlatforms,
    );
    _removeUnsupportedOnlinePlatforms();
    savedMenuIds.v = List<String>.from(
      json['savedMenuIds'] ?? HomeMenu.values.map((e) => e.id).toList(),
    );
  ...
  static Map<String, dynamic> extractConfig(Map<String, dynamic>? rootConfig) {
    ...
    return {
      ...
      'asmrSleepMinutes': (((app['asmrSleepMinutes'] as num?)?.toInt() ?? 60).clamp(
        1,
        maxSleepMinutes,
      )).toInt(),
