...
class WindowsAutoStart {
  ...
  static bool isEnabled() {
    ...
    try {
      ...
      if (RegOpenKeyEx(HKEY_CURRENT_USER, PCWSTR(pathPtr), 0, KEY_READ, lpKey.cast()) ==
          ERROR_SUCCESS) {
        final hKey = HKEY(Pointer.fromAddress(lpKey.value));
        if (RegQueryValueEx(hKey, PCWSTR(namePtr), nullptr, lpData.cast(), lpcbData) ==
            ERROR_SUCCESS) {
          enabled = true;
        }
        RegCloseKey(hKey);
  ...
  static bool enable() {
    ...
    try {
      ...
      if (RegOpenKeyEx(HKEY_CURRENT_USER, PCWSTR(pathPtr), 0, KEY_SET_VALUE, lpKey.cast()) ==
          ERROR_SUCCESS) {
        final hKey = HKEY(Pointer.fromAddress(lpKey.value));
        final result = RegSetValueEx(
          hKey,
          PCWSTR(namePtr),
          const REG_VALUE_TYPE(REG_SZ),
          exePath.cast(),
          (exePath.length + 1) * 2,
        );
